...
export default function BarRace({
  ...
  useEffect(() => {
    ...
    // Create color scale
    const color = d3.scaleOrdinal(d3.schemeTableau10).domain(games);
    ...
    const datevalues = Array.from(
      ...
      .map(([date, data]): [Date, d3.InternMap<string, number>] => [
        new Date(date),
        data,
      ])
    ...
    for (let i = 0; i < datevalues.length - 1; ++i) {
      ...
      for (let t = 0; t < k; ++t) {
        ...
        keyframes.push([
          ...
          rank(
            (name) =>
              (a.get(name) || 0) * (1 - alpha) + (b.get(name) || 0) * alpha
          ),
        ]);
      }
    }
    ...
    const barsG = svg.append("g").attr("fill-opacity", 0.6);
    ...
    const animate = async () => {
      while (animationRef.current <= keyframes.length) {
        if (animationRef.current === keyframes.length) {
            animationRef.current = 0;
        }
        ...
        // Update axis
        axisTransition(d3.transition().duration(duration).ease(d3.easeLinear));
        ...
        labelsG
          ...
          .each(function (d) {
            d3.select(this).select("tspan").text(formatNumber(d.value));
          });
    ...
    // Store animate function in ref so we can access it from reset
        ...
        <div className="relative flex items-center select-none touch-none w-72 h-5">
          <div className="relative flex-grow h-[6px] rounded-full bg-gray-800/20 border border-white/20">
            <div
              ...
              style={{
                width: `${(currentFrame / ((data.weeks.length - 1) * 7)) * 100}%`,
              }}
            />
          </div>
          <input
